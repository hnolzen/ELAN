...
cols <- c("Regional welfare" = "steelblue3", "Number of wind power plants" = "gray10")

p1 <- ggplot(data = m, aes(x = tick)) + 
  geom_line(size = 0.6, aes(y = welfare2, colour = "Regional welfare")) +
  geom_line(size = 1.0, aes(y = wind2, colour = "Number of wind power plants")) + 
  geom_hline(yintercept = 0, linetype = "dashed", size = 0.75) +
  
  geom_vline(xintercept =  52, linetype = "solid", size = 0.4) +
  geom_vline(xintercept = 104, linetype = "solid", size = 0.4) +
  geom_vline(xintercept = 155, linetype = "solid", size = 0.4) +
  geom_vline(xintercept = 240, linetype = "solid", size = 0.4) +

  labs(x = "Time step [-]", 
       y = "Regional welfare [MU]", 
       color = "") + 
  coord_cartesian(ylim = c(-250, 480)) +
  ...
  annotate("text", x = 480, y = 465, label = "(b)", size = 12, fontface = 2) + 
  guides(color = guide_legend(override.aes = list(linewidth = 3)))
  
ggsave(filename = "c3_single_region2.pdf", device = "pdf",
