...
df <- read.table(file = "C2-tabu-north.csv", sep = ",", skip = 7)
df6 <- read.table(file = "C2-tabu-north-600.csv", sep = ",", skip = 7)
...
p3 <- ggplot(data = df, aes(x = df$tabu, y = df$disutility)) + 
  ...
  annotate("text", x = 19.3, y = 6050, label = "(b)", size = 12, fontface = 2) + 
  annotate(geom="label", x = 10.5, y = 6025, label = TeX('$d_{h} = 4'), size = 12, fill = "white") +
  guides(shape = FALSE, linetype = FALSE)

ggsave(filename = "c2_tabu_north_var.pdf", device = "pdf",
