...
df <- read.table(file = "C2-tabu-clusters.csv", sep = ",", skip = 7)
df6 <- read.table(file = "C2-tabu-clusters-600.csv", sep = ",", skip = 7)
...
p3 <- ggplot(data = df, aes(x = df$tabu, y = df$disutility)) + 
  ...
  annotate("text", x = 19.3, y = 6050, label = "(c)", size = 12, fontface = 2) + 
  annotate(geom="label", x = 10.5, y = 6025, label = TeX('$d_{h} = 4'), size = 12, fill = "white") +
  guides(shape = FALSE, linetype = FALSE)

ggsave(filename = "c2_tabu_clusters_var.pdf", device = "pdf",
