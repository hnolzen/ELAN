...
df <- subset(df, tabu == 4 | tabu == 8 | tabu == 12 | tabu == 16)
df6 <- subset(df6, tabu == 4 | tabu == 8 | tabu == 12 | tabu == 16)
...
p1 <- ggplot(data = df, aes(x = df$demand, y = df$disutility, colour = as.factor(tabu))) + 
  ...
  scale_color_manual(values = c("#fd8d3c", "#31a354", "#2c7bb6", "#fc0356")) +
...
ggsave(filename = "c2_tabu_mixed_e.pdf", device = "pdf",
