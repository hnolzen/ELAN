...
df <- read.table(file = "C2-netcosts-clusters.csv", sep = ",", skip = 7)
df6 <- read.table(file = "C2-netcosts-clusters-600.csv", sep = ",", skip = 7)
...
p3 <- ggplot(data = df, aes(x = df$netcosts, y = df$disutility)) + 
  ...
  annotate("text", x = 487, y = 1985, label = "(c)", size = 12, fontface = 2) + 
  annotate(geom="label", x = 300, y = 1970, label = TeX('$d_{h} = 4'), size = 12, fill = "white") + 
  guides(shape = FALSE, linetype = FALSE)

ggsave(filename = "c2_ncw_clusters_var.pdf", device = "pdf",
