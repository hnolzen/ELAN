...
df <- read.table(file = "C2-netcosts-south.csv", sep = ",", skip = 7)
df6 <- read.table(file = "C2-netcosts-south-600.csv", sep = ",", skip = 7)
...
p3 <- ggplot(data = df, aes(x = df$netcosts, y = df$disutility)) + 
  ...
  ggtitle("South cluster") +
  labs(y = "Number occupied grid cells [-]",
       x = "",
       shape = "",
       linetype = "") +
  ...
  theme(plot.title = element_text(size = 40,  hjust = 0.5, face = "bold"),
        ...
        axis.title.x = element_text(colour = "black", size = 32),
  ...
  annotate("text", x = 487, y = 1985, label = "(a)", size = 12, fontface = 2) + 
  annotate(geom="label", x = 300, y = 1970, label = TeX('$d_{h} = 4'), size = 12, fill = "white") + 
  guides(shape = FALSE, linetype = FALSE)

ggsave(filename = "c2_ncw_south_var.pdf", device = "pdf",
